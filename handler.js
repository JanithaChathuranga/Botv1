...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (opts["pconly"] && m.chat.endsWith("g.us"))
            return
        if (opts["gconly"] && !m.chat.endsWith("g.us") && ![conn.decodeJid(global.conn.user.id), ...global.owner.map(([number]) => number)].map(v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender))
            return
            ...
            if ((usedPrefix = (match[0] || "")[0])) {
                ...
                if (xp > 200)
                    this.sendMessage(m.chat, {
                        text: `[❗] *It Looks Like Youre Playing Cheat Using Calculator*`,
                        mentions: [m.sender]
                    }, {
                        quoted: m
                    })
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.sendMessage(m.chat, {
                        text: `[❗] *Your limit has run out some commands cannot be accessed*`,
                        mentions: [m.sender]
                    }, {
                        quoted: m
                    })
...
const byeran = await conn.profilePictureUrl(user, 'image')
const welthum = await WelcomeLeave(await this.profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), await this.getName(user), katarandom.getRandom());
 const byethum = await WelcomeLeave(await this.profilePictureUrl(id, "image").catch(() => hwaifu.getRandom()), await this.getName(id), katarandom.getRandom());
const lapor = `\n\n${emoji.mail} *Pesan:* If you find any bugs errors or difficulties in use please report/ask ${emoji.owner}`;
 const welimg = isAddAction ? welran : byeran
await this.sendMessage(id, {
   //image: await (await fetch(welimg)).buffer(),
       //mimetype: 'image/jpeg',
        //fileLength: 10630044057600000000000000000000,
   ...
   body: global.author, 
   mediaType: 1, 
   thumbnail: await (await conn.getFile(isAddAction ? welthum : byethum)).data, 
...
//conn.sendMessage(id, {image : {url:welthum}})
 /*await this.sendFile(id, isAddAction ? welthum : byethum, '', welcomeText.replace("@user", "@" + participants[0].split("@")[0]) + lapor, fakes, null, {
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) {
            ...
            text = (chats.sIcon || this.sIcon || conn.sIcon || `*${emoji.icon} Icon has been changed to*`)
            ...
            text = (chats.sAnnounceOn || this.sAnnounceOn || conn.sAnnounceOn || `*${emoji.announceOn} The group has been closed!*`)
        } else if (groupUpdate.announce === false) {
            text = (chats.sAnnounceOff || this.sAnnounceOff || conn.sAnnounceOff || `*${emoji.announceOff} Groups are open!*`)
        } else if (groupUpdate.restrict === true) {
            text = (chats.sRestrictOn || this.sRestrictOn || conn.sRestrictOn || `*${emoji.restrictOn} Groups have been limited to participants only!*`)
        } else if (groupUpdate.restrict === false) {
            text = (chats.sRestrictOff || this.sRestrictOff || conn.sRestrictOff || `*${emoji.restrictOff} The group has been restricted to admins only!*`)
        }
...
global.dfail = async (type, m, conn) => {
    ...
    const msg = {
        owner: `*${emoji.owner} ᴏᴡɴᴇʀ*\n
${userTag} This command can only be used by *Owner Bot* !`,
        moderator: `*${emoji.moderator} ᴍᴏᴅᴇʀᴀᴛᴏʀ*\n
${userTag} This command can only be used by *Moderators* !`,
        premium: `*${emoji.premium} ᴘᴇʀʜᴀᴛɪᴀɴ ᴘʀᴇᴍɪᴜᴍ*\n
${userTag} This order is for members only *Premium* !`,
        group: `*${emoji.group} ᴘᴇʀʜᴀᴛɪᴀɴ ɢʀᴜᴘ*\n
${userTag} This command can only be used in groups !`,
        private: `*${emoji.private} ᴘᴇʀʜᴀᴛɪᴀɴ ᴘʀɪᴠᴀᴛᴇ*\n
${userTag} This command can only be used in Private Chat !`,
        admin: `*${emoji.admin} ᴘᴇʀʜᴀᴛɪᴀɴ ᴀᴅᴍɪɴ*\n
${userTag} This command is only for *Admin* grup !`,
        botAdmin: `*${emoji.botAdmin} ᴘᴇʀʜᴀᴛɪᴀɴ ʙᴏᴛ ᴀᴅᴍɪɴ*\n
${userTag} Make bots as *Admin* to use this command !`,
...
${userTag} NSFW is not active please contact the Bot Discussion Team to activate this feature !`,
        rpg: `*${emoji.rpg} ᴘᴇʀʜᴀᴛɪᴀɴ ʀᴘɢ*\n
${userTag} RPG is not active please contact Team Bot Discussion to activate this feature !`,
