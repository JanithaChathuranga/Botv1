...
//const emojiAndSymbolRegex = new RegExp(`^(?:${symbolRegex.source}|${emojiRegex().source})`, 'u');
//global.prefix = /^/;
//global.prefix = /^/; // Ekspresi reguler kosong yang memungkinkan perintah dimulai tanpa karakter awalan
...
if (pairingCode && !conn.authState.creds.registered) {
    ...
    console.log(`📨 ${chalk.redBright('masukkan nomor whatsapp')}:`);
    console.log(chalk.cyan('├──────────────────────────────────────···'));
...
global.reloadHandler = async function(restatConn) {
    ...
    conn.welcome = `${emoji.welcome} Hallo @user\n\n   *W E L C O M E*\n⫹⫺ In the group @subject\n\n⫹⫺ Read *DESCRIPTION*\n@desc`;
    conn.bye = `   *G O O D B Y E*\n${emoji.bye} See you @user`;
    conn.spromote = `*${emoji.promote} @user* now become an admin!`;
    conn.sdemote = `*${emoji.demote} @user* no longer an admin!`;
    conn.sDesc = `${emoji.desc} Description has been changed to:\n@desc`;
    conn.sSubject = `${emoji.subject} The group title has been changed to:\n@subject`;
...
filesInit()
    .then(async (_) => {
        ...
        return await conn.sendMessage(nomorown + "@s.whatsapp.net", {
                text: "ʙᴏᴛ ꜱᴜᴄᴄᴇꜱꜱғᴜʟʟʏ ʀᴇʟᴏᴀᴅ ᴄᴏᴍᴍᴀɴᴅ"
            }, {
                quoted: null
            })
            .catch(() => {
...
const actions = [{
    ...
    {
        func: purgeSession,
        message: '\nSaved Sessions Have Been Deleted ✅'
    },
    {
        func: purgeSessionSB,
        message: '\nSub-Bot Sessions Have Been Deleted ✅'
    },
    {
        func: purgeOldFiles,
